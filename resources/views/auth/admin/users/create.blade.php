...
@section('content')
    ...
    <div class="admin-content-wrapper">
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="course-form" novalidate>
            @csrf

            <div class="form-group">
                <label for="first_name">Имя</label>
                <input id="first_name" name="first_name" type="text" value="{{ old('first_name') }}" required class="{{ $errors->has('first_name') ? 'input-error' : '' }}">
                @if($errors->has('first_name'))<div class="error">{{ $errors->first('first_name') }}</div>@endif
            </div>

            <div class="form-group">
                <label for="last_name">Фамилия</label>
                <input id="last_name" name="last_name" type="text" value="{{ old('last_name') }}" required class="{{ $errors->has('last_name') ? 'input-error' : '' }}">
                @if($errors->has('last_name'))<div class="error">{{ $errors->first('last_name') }}</div>@endif
            </div>

            <div class="form-group">
                <label for="email">Email</label>
                <input id="email" name="email" type="email" value="{{ old('email') }}" required class="{{ $errors->has('email') ? 'input-error' : '' }}">
                @if($errors->has('email'))<div class="error">{{ $errors->first('email') }}</div>@endif
            </div>

            <div class="form-group">
                <label for="phone">Телефон</label>
                <input id="phone" name="phone" type="text" value="{{ old('phone') }}" class="{{ $errors->has('phone') ? 'input-error' : '' }}">
                @if($errors->has('phone'))<div class="error">{{ $errors->first('phone') }}</div>@endif
            </div>

            <div class="form-group">
                <label for="date_birthday">Дата рождения</label>
                <input id="date_birthday" name="date_birthday" type="date" value="{{ old('date_birthday') }}" class="{{ $errors->has('date_birthday') ? 'input-error' : '' }}">
            ...
            <div class="form-group">
                ...
                <select id="level" name="level" required class="{{ $errors->has('level') ? 'input-error' : '' }}">
                    <option value="" disabled {{ old('level') ? '' : 'selected' }}>Выберите уровень</option>
                    @foreach(['beginner','A1','A2','B1','B2','C1','C2'] as $lvl)
                        <option value="{{ $lvl }}" {{ old('level')==$lvl ? 'selected' : '' }}>{{ $lvl=='beginner' ? 'начинающий' : $lvl }}</option>
                    @endforeach
                </select>
                @if($errors->has('level'))<div class="error">{{ $errors->first('level') }}</div>@endif
            </div>

            <div class="form-group">
                <label for="description">Описание</label>
                <textarea id="description" name="description" rows="4" class="{{ $errors->has('description') ? 'input-error' : '' }}">{{ old('description') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input id="password" name="password" type="password" required class="{{ $errors->has('password') ? 'input-error' : '' }}">
                @if($errors->has('password'))<div class="error">Минимум 6 символов</div>@endif
            </div>
