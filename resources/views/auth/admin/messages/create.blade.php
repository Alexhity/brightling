...
@section('content')
    ...
    <div class="admin-content-wrapper">
        <form action="{{ route('admin.messages.store') }}"
              method="POST"
              class="message-form"
              novalidate>
            @csrf
                ...
                <select name="recipient_id" id="recipient_id"
                    ...
                    <optgroup label="Конкретные пользователи">
                        @foreach($allUsers as $u)
                            @continue($u->id === auth()->id()) {{-- пропускаем себя --}}
                            <option value="{{ $u->id }}">
            ...
            <div class="form-group">
                ...
                <textarea name="question_text" id="question_text" rows="5"
                          class="@error('question_text') input-error @enderror" required>{{ old('question_text') }}</textarea>
